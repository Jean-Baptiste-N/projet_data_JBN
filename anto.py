...
def load_data():
    try:
        ...
        # Liste des requêtes et leurs descriptions
        queries = [
            {
            ...
            },
            {
                ...
                'progress': 25,
                'query': '''SELECT * FROM `projet-jbn-data-le-wagon.dbt_medical_analysis_nbr_hospi_par_tranche_age.int_nbr_hospi_dpt_ens_par_tranche_age`''',
                'df_name': 'df_nbr_hospi'
            },
            {
                'text': "Chargement des durées d'hospitalisation...",
                'progress': 50,
                'query': '''SELECT * FROM `projet-jbn-data-le-wagon.duree_hospitalisation_par_patho.duree_hospi_region_et_dpt_clean_classifie`''',
                'df_name': 'df_duree_hospi'
            },
            {
                'text': "Chargement des données par tranche d'âge...",
                'progress': 75,
                'query': '''SELECT * FROM `projet-jbn-data-le-wagon.morbidite_h.tranche_age_intermediate`''',
                'df_name': 'df_tranche_age_hospi'
            },
            {
                'text': "Chargement des capacités hospitalières...",
                'progress': 90,
                'query': '''SELECT * FROM `projet-jbn-data-le-wagon.capacite_services_h.jointure_capa_hospi_dureehospi_KPIs`''',
        ...
        # Connexion initiale
        progress_bar.progress(0, text=queries[0]['text'])
        gcp_service_account = st.secrets["gcp_service_account"]
        client = bigquery.Client.from_service_account_info(gcp_service_account)
        
        # Chargement des données
        for query_info in queries[1:]:  # Skip the first item (connection)
            progress_bar.progress(query_info['progress'], text=query_info['text'])
            df = client.query(query_info['query']).to_dataframe()
            if query_info['df_name'] == 'df_nbr_hospi':
                df_nbr_hospi = df
            elif query_info['df_name'] == 'df_duree_hospi':
                df_duree_hospi = df
            elif query_info['df_name'] == 'df_tranche_age_hospi':
                df_tranche_age_hospi = df
            elif query_info['df_name'] == 'df_capacite_hospi':
