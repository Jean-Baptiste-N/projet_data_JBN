...
# Titre principal
st.markdown ("<h1 class='main-title' style='margin-top: -70px;'>👶 Service Obstétrique</h1>", unsafe_allow_html=True)
...
if df is not None:
    ...
    with col1:
        ...
        selected_sex = st.selectbox(
            "Sexe",
            ["Ensemble", "Femme"],  # Obstétrique concerne principalement les femmes
    ...
    search_term = st.text_input("🔍 Rechercher une pathologie obstétrique pour obtenir des détails", "")
    ...
    if search_term:
        ...
        if filtered_pathologies:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.metric("Nombre total d'hospitalisations en obstétrique", f"{total_hospi:,.0f}")
            with col2:
    ...
    with col_help:
        st.metric(
            ...
            - Total des hospitalisations : nombre total d'hospitalisations en obstétrique
            - Durée moyenne : temps moyen de séjour en service obstétrique
            - Évolution : tendance des hospitalisations par rapport à la période précédente
            
            Note : Les durées de séjour en obstétrique sont généralement plus courtes que dans les autres services, sauf en cas de complications."""
        )

    st.divider()
    
    # Ajout d'un sélecteur pour filtrer le nombre de pathologies à afficher
    n_pathologies = st.slider("Nombre de pathologies à afficher", 3, 14, 10)
    ...
    with col_help:
        st.metric(label="help", value="", help="Ce graphique montre la relation entre le nombre d'hospitalisations (barres) et la durée moyenne de séjour (ligne) pour les pathologies obstétriques les plus fréquentes.")

    # Tableau récapitulatif détaillé
    st.subheader("Évolution des pathologies obstétriques (2018-2022)")
