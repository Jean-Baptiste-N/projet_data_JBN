...
def load_data():
    try:
        ...
        files = [
            {
                'text': "Chargement des données d'hospitalisation...",
                'progress': 25,
                'file': 'int_nbr_hospi_dpt_ens_par_tranche_age.csv',
                'df_name': 'df_nbr_hospi'
            },
            {
                'text': "Chargement des durées d'hospitalisation...",
                'progress': 50,
                'file': 'duree_hospi_region_et_dpt_clean_classifie.csv',
                'df_name': 'df_duree_hospi'
            },
            {
                'text': "Chargement des données par tranche d'âge...",
                'progress': 75,
                'file': 'int_nbr_hospi_dpt_ens_par_annee.csv',
                'df_name': 'df_tranche_age_hospi'
            },
            {
                'text': "Chargement des capacités hospitalières...",
                'progress': 90,
                'file': 'jointure_capa_hospi_dureehospi_KPIs.csv',
                'df_name': 'df_capacite_hospi'
            }
        ]
        
        # Initialisation des DataFrames
        df_nbr_hospi = df_duree_hospi = df_tranche_age_hospi = df_capacite_hospi = None
        
        # Chargement des données
        for file_info in files:
            progress_bar.progress(file_info['progress'], text=file_info['text'])
            file_path = os.path.join(dataset_path, file_info['file'])
            df = pd.read_csv(file_path)
            
            if file_info['df_name'] == 'df_nbr_hospi':
                df_nbr_hospi = df
            elif file_info['df_name'] == 'df_duree_hospi':
                df_duree_hospi = df
            elif file_info['df_name'] == 'df_tranche_age_hospi':
                df_tranche_age_hospi = df
            elif file_info['df_name'] == 'df_capacite_hospi':
                df_capacite_hospi = df
...
def calculate_main_metrics(df_nbr_hospi, df_capacite_hospi):
    ...
    for year in range(2018, 2023):
        total_hospi = df_nbr_hospi["tranche_age_tous_ages"][pd.to_datetime(df_nbr_hospi["year"]).dt.year == year].sum()
...
if df_nbr_hospi is not None:
    ...
    #Filtre région
    region = sorted(df_capacite_hospi['region'].unique())
    select_all_region = st.sidebar.checkbox("Sélectionner toutes les régions", value=True)
    if select_all_region:
        selected_region = st.sidebar.multiselect("Sélectionner les régions", region, default=region)
    else:
        selected_region = st.sidebar.multiselect("Sélectionner les régions", region)
    # Filtre départements
    ...
    with tab1:
        ...
        with col1:
            hospi_by_year = df_nbr_hospi.groupby('year')['tranche_age_tous_ages'].sum().reset_index()
            fig = px.line(hospi_by_year, x='year', y='tranche_age_tous_ages',
                         title='Nombre d\'hospitalisations par année',
                         labels={'year': 'Année', 'tranche_age_tous_ages': 'Nombre d\'hospitalisations'},
                         custom_data=['year', 'tranche_age_tous_ages'])
            fig.update_traces(
    ...
    with tab2:
        ...
        col1, col2 = st.columns(2)
        with col1:
            hospi_by_departement = df_nbr_hospi_filtered.groupby('nom_departement')['tranche_age_tous_ages'].sum().reset_index()
            hospi_by_departement = hospi_by_departement.sort_values(by='tranche_age_tous_ages', ascending=True)
            fig = px.bar(hospi_by_departement, x='tranche_age_tous_ages', y='nom_departement',
                        title='Nombre d\'hospitalisations par département',
                        labels={'tranche_age_tous_ages': 'Nombre d\'hospitalisations',
                               'nom_departement': 'Département'},
                        custom_data=['nom_departement', 'tranche_age_tous_ages'],
                        orientation='h')
            fig.update_traces(
    ...
    with tab3:
        ...
        n_pathologies = st.slider("Sélectionne les pathologies les plus fréquentes", 5, 50, 20)
        
        # Top pathologies par nombre d'hospitalisations
        hospi_by_pathology = df_nbr_hospi_filtered.groupby('nom_pathologie')['tranche_age_tous_ages'].sum().reset_index()
        hospi_by_pathology = hospi_by_pathology.sort_values(by='tranche_age_tous_ages', ascending=True).tail(n_pathologies)
        fig = px.bar(hospi_by_pathology, x='tranche_age_tous_ages', y='nom_pathologie',
                    title=f'Top {n_pathologies} Pathologies par nombre d\'hospitalisations',
                    labels={'tranche_age_tous_ages': 'Nombre d\'hospitalisations',
                           'nom_pathologie': 'Pathologie'},
                    custom_data=['nom_pathologie', 'tranche_age_tous_ages'],
                    orientation='h')
        fig.update_traces(
...
def prepare_map_data(hospi_by_departement, hospi_by_region):
    ...
    dept_map_data = dict(zip(hospi_by_departement['nom_departement'], hospi_by_departement['tranche_age_tous_ages']))
    region_map_data = dict(zip(hospi_by_region['nom_region'], hospi_by_region['tranche_age_tous_ages']))
...
def generate_multi_level_map(dept_map_data, region_map_data, dept_geojson, region_geojson, selected_view):
    ...
    if selected_view == "Régions":
        ...
        region_choropleth = folium.Choropleth(
            ...
            columns=['nom_region', 'tranche_age_tous_ages'],
        ...
        dept_choropleth = folium.Choropleth(
            ...
            data=dept_map_data,
            columns=['nom_departement', 'tranche_age_tous_ages'],
            key_on="feature.properties.nom",
            fill_color="YlOrRd",
            fill_opacity=0.7,
            line_opacity=0.2,
...
with tab5:
    ...
    # Préparation des données pour la carte
    hospi_by_departement = df_capacite_hospi_filtered.groupby('nom_departement')['tranche_age_tous_ages'].sum().reset_index()
    hospi_by_region = df_capacite_hospi_filtered.groupby('nom_region')['tranche_age_tous_ages'].sum().reset_index()
