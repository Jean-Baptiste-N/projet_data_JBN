WITH transposed_data AS (
    SELECT 
        niveau,
        CONCAT(departement,"_",code_pathologie) as cle_unique,
        pathologie,
        code_pathologie,
        nom_pathologie,
        departement,
    ...
    FROM {{ ref("int_duree_sejours_dpt_ens") }}
)
SELECT
    niveau,
    cle_unique,
...
GROUP BY niveau, cle_unique, pathologie, code_pathologie, nom_pathologie, departement, code_departement, nom_departement
ORDER BY cle_unique
